//! TontooUI TextField demo — CapsuleTextField directly on window.

use tontooui::prelude::*;
use tontooui::CapsuleTextField;
...
fn main() {
    let mut app = App::new("TontooUI TextField", 1180, 900);
    let title = Text::new("TextField").font_size(18.0).bold().color(Color::from_hex("#7dd3e0").unwrap());

    let row = HStack::new().spacing(24.0).child(cell(
        "Capsule Text Field",
        "Gives your text field a capsule shape. This only has a visible effect on macOS.",
        "modifier",
        CapsuleTextField::new(),
    ));
