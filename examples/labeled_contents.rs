//! TontooUI LabeledContent demo — all 3 LabeledContent elements directly on window.

use tontooui::prelude::*;
use tontooui::{CustomLabeledContent, FormattedLabeledContent, LabeledContent};
...
fn main() {
    let mut app = App::new("TontooUI LabeledContent", 1180, 900);
    let title = Text::new("LabeledContent").font_size(18.0).bold().color(Color::from_hex("#7dd3e0").unwrap());

    let row = HStack::new()
        .spacing(24.0)
        .child(cell(
            "Custom Labeled Content",
            "Creates a standard labeled element, with a view that conveys the value.",
            "initializer",
            CustomLabeledContent::new(),
        ))
        .child(cell(
            "Formatted Labeled Content",
            "Creates a labeled informational view from a formatted value.",
            "initializer",
            FormattedLabeledContent::new(),
        ))
        .child(cell(
            "Labeled Content",
