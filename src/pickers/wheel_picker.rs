...
fn item_factor(dist: f32, item_h: f32) -> f32 {
    (dist * item_h / FADE_DISTANCE).clamp(0.0, 1.0)
}
...
fn style_item(label: &Label, dist: f32, item_h: f32) {
    let factor = item_factor(dist, item_h);
...
fn layout_items(state: &WheelState, labels: &[Label], h: f32, item_h: f32) {
...
impl WheelPicker {
    ...
    /// Set the row height for the wheel drum (SwiftUI `wheelPickerItemHeight`).
    pub fn item_height(mut self, height: f32) -> Self {
        self.item_height = height.max(16.0);
        self
    }

    /// Alias for `item_height` (mirrors SwiftUI modifier name).
    pub fn wheel_item_height(self, height: f32) -> Self {
...
impl ViewContent for WheelPicker {
    fn render(&self, frame: Rect) -> gtk::Widget {
        ...
        uikit::widget::apply_css(
            &highlight,
            ".wheel-highlight { background: rgba(255, 255, 255, 0.10); border-top: 1px solid rgba(255, 255, 255, 0.04); border-bottom: 1px solid rgba(255, 255, 255, 0.04); border-radius: 12px; }",
        );
        ...
        if num_items > 0 {
            layout_items(&state.borrow(), &labels, h, item_h);
        }
...
mod tests {
    ...
    fn wheel_visual_helpers() {
        assert_eq!(item_factor(0.0, ITEM_HEIGHT), 0.0);
        assert!((item_opacity(item_factor(0.0, ITEM_HEIGHT)) - 1.0).abs() < 1e-4);
        // An item fully faded (1.75 rows away) keeps 35% opacity.
        assert!((item_opacity(item_factor(1.75, ITEM_HEIGHT)) - 0.35).abs() < 1e-4);
        // Center scale is 1.0, edge scale the minimum.
        assert!((item_scale(item_factor(0.0, ITEM_HEIGHT)) - 1.0).abs() < 1e-4);
        assert!((item_scale(item_factor(1.75, ITEM_HEIGHT)) - MIN_SCALE).abs() < 1e-4);
    }

    #[test]
    fn wheel_picker_item_height() {
        let p = WheelPicker::new().items(["A","B"]).item_height(52.0);
        assert!((p.item_height - 52.0).abs() < 1e-4);
        let p2 = WheelPicker::new().items(["A","B"]).wheel_item_height(28.0);
        assert!((p2.item_height - 28.0).abs() < 1e-4);
    }
}
