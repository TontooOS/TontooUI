...
use super::{
    SHAPE_CORNER_RADIUS, SHAPE_DEFAULT_FILL, SHAPE_DEFAULT_STROKE, SHAPE_SHADOW,
    SHAPE_SHADOW_DY, SHAPE_STROKE_W, ShapeFill, fill_brush, stroke_color,
};
use crate::renderer::images::ImageLoader;
use crate::renderer::text::FontSystem;

/// Rectangle with rounded corners, optional gradient fill and outline
/// look. Display-only (no mouse handling).
...
impl RoundedRectangle {
    ...
    pub fn corner_radius(mut self, px: f32) -> Self {
        self.corner_radius = px.max(0.0);
        self
    }
...
impl Default for RoundedRectangle {
    fn default() -> Self {
        Self::new(200.0, 120.0, SHAPE_CORNER_RADIUS)
    }
}
...
mod tests {
    ...
    fn keeps_intrinsic_size() {
        ...
        let mut shape = RoundedRectangle::new(200.0, 120.0, 16.0);
        assert_eq!(shape.measure(&mut fonts), (200.0, 120.0));
        shape.place(&mut fonts, 0.0, 0.0, 200.0, 120.0);
        assert_eq!(shape.rect(), (0.0, 0.0, 200.0, 120.0));
    }

    #[test]
    fn clamps_radius_to_half_side() {
        let mut shape = RoundedRectangle::new(100.0, 40.0, 999.0);
        let mut fonts = FontSystem::new();
        shape.place(&mut fonts, 0.0, 0.0, 100.0, 40.0);
        assert_eq!(shape.radius(), 20.0);
    }

    #[test]
    fn clamps_negative_values() {
        let shape = RoundedRectangle::new(-10.0, -5.0, -4.0).stroke_width(-2.0);
        assert_eq!(shape.stroke_width_value(), 0.0);
        assert_eq!(shape.corner_radius_value(), 0.0);
    }
}
