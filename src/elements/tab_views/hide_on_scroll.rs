//! HideTabBarOnScrollDown — Minimize the tab bar when downwards scrolling starts. Minimizing is supporte...
...
/// HideTabBarOnScrollDown — modifier — Minimize the tab bar when downwards scrolling starts. Minimizing is supporte...
...
impl HideTabBarOnScrollDown {
    ...
    pub fn to_view(self) -> View {
        View::new(self).with_frame(0.0, 0.0, 180.0, 60.0)
    }
}

impl Default for HideTabBarOnScrollDown { fn default() -> Self { Self::new() } }

impl ViewContent for HideTabBarOnScrollDown {
    fn render(&self, frame: Rect) -> gtk::Widget {
        ...
        if frame.width > 0.0 { outer.set_size_request(frame.width as i32, 60); }

        let title = gtk::Label::new(Some("HideTabBarOnScrollDown"));
        ...
        let hint = gtk::Label::new(Some("Minimize the tab bar when downwards sc"));
    ...
    fn size_that_fits(&self, _: Size) -> Size { Size::new(180.0, 60.0) }
}

impl Widget for HideTabBarOnScrollDown {
    fn id(&self) -> WidgetId { self.id }
    fn position_mode(&self) -> PositionMode { self.position_mode }
    fn position(&self) -> Position { self.position }
    fn to_gtk(&self) -> gtk::Widget { self.render(Rect::new(0.0,0.0,180.0,60.0)) }
...
mod tests {
    use super::*;
    #[test]
    fn hide_on_scroll_exists() { let _ = HideTabBarOnScrollDown::new(); }
}
